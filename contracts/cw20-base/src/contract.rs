...
pub fn execute_batch_transfer(
    ...
    if recipients.len() != amounts.len()  {
        return Err(ContractError::InvalidLengthOfTwoVectors {  });
    }

    let mut total_amount = Uint128::new(0);
    for (pos, recipient) in recipients.iter().enumerate() {
        let rcpt_addr = deps.api.addr_validate(&recipient)?;
        total_amount += amounts[pos];

        //increase the balance of recepient
        BALANCES.update(
            deps.storage,
            &rcpt_addr,
            |balance: Option<Uint128>| -> StdResult<_> {Ok(balance.unwrap_or_default() + amounts[pos])},
        )?;
    }
    ...
    BALANCES.update(
        ...
        &info.sender,
        |balance: Option<Uint128>| -> StdResult<_> {
            Ok(balance.unwrap_or_default().checked_sub(total_amount)?)
        },
    )?;
